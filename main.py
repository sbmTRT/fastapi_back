...
async def verify_line_token(access_token: str):
...
async def check_token(access_token: str):
    if not access_token:
        raise HTTPException(status_code=400, detail="LINE access token is required")

    try:
        verify_response = await verify_line_token(access_token)
        # If you reach this point, the token is valid
        return {"valid": True, "verify_response": verify_response}
